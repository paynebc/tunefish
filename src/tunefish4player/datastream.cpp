...
void eDataStream::WriteBits(eU64 val, eU32 bitCount)
{
  eASSERT(bitCount <= 64);
...
void eDataStream::WriteU64(eU64 val)
{
  WriteBits(val, 64);
}
...
eU64 eDataStream::ReadBits(eU32 bitCount)
{
  eU64 res = 0;
  for (eU32 i = 0; i < bitCount; i++)
    //        res |= ((eU64)readBit()<<i);
